...
class FastSegmentation():
    def __init__(self,config):
        self.logger = Logger("FastSegmentation-" + str(self.__hash__()), ic=IconMode.left_right, ic_color=IconColor.magenta)
    ...
    def process(self,image):
        ...
        pred = torch.argmax(F.interpolate(logit, size=(1024,2048), mode='bilinear', align_corners=True), dim=-3)
        pred = pred.squeeze()