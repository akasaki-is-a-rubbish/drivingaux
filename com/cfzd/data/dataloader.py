...
import com.cfzd.data.mytransforms as mytransforms
from com.cfzd.data.constant import tusimple_row_anchor, culane_row_anchor
from com.cfzd.data.dataset import LaneClsDataset, LaneTestDataset
...
def get_test_loader(batch_size, data_root,dataset, distributed):
    ...
    if dataset == 'CULane':
        test_dataset = LaneTestDataset(data_root,os.path.join(data_root, 'list/Thz_test.txt'),img_transform = img_transforms)
        cls_num_per_lane = 18
    elif dataset == 'Tusimple':
        test_dataset = LaneTestDataset(data_root,os.path.join(data_root, 'Thz_test.txt'), img_transform = img_transforms)
        cls_num_per_lane = 56
